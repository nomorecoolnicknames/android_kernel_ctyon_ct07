...
{    
    ...
    *buf = 0;
    ...
    obj = (struct mir3da_i2c_data *)i2c_get_clientdata(client);
    if (NULL == obj)
        return -ENODEV;
...
/*----------------------------------------------------------------------------*/
static int mir3da_store_factory_calibration(struct i2c_client *client)
{
	char buf[MIR3DA_BUFSIZE] = {0};
	mm_segment_t old_fs;
	struct file *filp;
	loff_t pos = 0;
	ssize_t written;
	...
	/* The shipped JNI expects decimal values in the stock z,x,y order. */
	scnprintf(buf, sizeof(buf), "%d %d %d", z, x, y);

	old_fs = get_fs();
	set_fs(KERNEL_DS);
	filp = filp_open("/data/mir3da", O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (IS_ERR(filp)) {
		ret = PTR_ERR(filp);
		goto restore_fs;
	}
	...
	written = filp->f_op->write(filp, buf, sizeof(buf), &pos);
	if (written < 0)
		ret = written;
...
static long mir3da_misc_ioctl( struct file *file,unsigned int cmd, unsigned long arg)
{
 ...
 printk("[mir3da_misc_ioctl] in!!!!! cmd===%d\n",cmd);
    ...
    case GSENSOR_IOCTL_READ_CHIPINFO:
		...
		err = mir3da_readChipInfo(client, strbuf, MIR3DA_BUFSIZE);
		if (err)
			break;
    ...
    case GSENSOR_IOCTL_READ_SENSORDATA:
        ...
        if(data == NULL)
        {
	        ...
	        err = mir3da_readSensorData(client, strbuf);
	        if (err)
	            break;
        if(copy_to_user(data, strbuf, strlen(strbuf)+1))
        {
    ...
    case GSENSOR_IOCTL_READ_RAW_DATA:
        ...
        if(data == NULL)
        {
	        ...
	        err = mir3da_readRawData(client, strbuf);
	        if (err)
	            break;
        if(copy_to_user(data, strbuf, strlen(strbuf)+1))
        {
	...
	case MIR3DA_ACC_IOCTL_CALIBRATION:
		MI_MSG("IOCTRL --- MIR3DA factory calibration\n");
		err = mir3da_store_factory_calibration(client);
		break;
...
static ssize_t mir3da_enable_store(struct device_driver *ddri, const char *buf, size_t count)
{
    ...
    ret = mir3da_setPowerMode(client, bEnable);
    if (ret < 0){
        ret = -EINVAL;
    }
    else{
...
static ssize_t mir3da_reg_data_store(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "0x%x, 0x%x", &addr, &data) != 2)
		return -EINVAL;
    ...
    switch (command)
    {
        ...
        case SENSOR_GET_DATA:
            ...
            else
            {
                ...
                err = mir3da_readSensorData(priv->client, buff);
                if (err)
                    break;
...
int i2c_smbus_read(PLAT_HANDLE handle, u8 addr, u8 *data)
{
    ...
    res = i2c_smbus_read_byte_data(client, addr);
    if (res < 0)
        return res;
...
static int mir3da_get_data(int *x , int *y, int *z, int *status) 
{
    ...
    ret = mir3da_readSensorData(mir_handle, buff);
    if (ret)
        return ret;
...
static int mir3da_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
 ...
 printk("mir3da_probe=========in!!!!!\n");
    ...
    i2c_set_clientdata(new_client,obj);	
	    ...
	    if (!obj->hw) {
	        result = -ENODEV;
	        MI_ERR("missing CT07 accelerometer platform data\n");
	        goto exit_kfree;
	    }
#else
#if MTK_AUTO_MODE
...
#endif
#endif 
	    if (!obj->hw->i2c_addr[0] || obj->hw->i2c_addr[0] > 0x7f) {
	        result = -EINVAL;
	        MI_ERR("invalid I2C address: 0x%02x\n", obj->hw->i2c_addr[0]);
	        goto exit_kfree;
	    }
	    new_client->addr = obj->hw->i2c_addr[0];
	    printk("mir3da_probe: i2c_number=%d i2c_addr=0x%02x\n",
	           obj->hw->i2c_num, new_client->addr);
	...
	i2c_smbus_read((PLAT_HANDLE) new_client, NSA_REG_WHO_AM_I, &chip_id);	
		if(chip_id != 0x13){
			client->addr = 0x27;
			printk("mir3da: chip_id != 0x13, trying address 0x27\n");
		}	
...
#if MTK_AUTO_MODE
	mir3da_delete_attr(&(mir3da_init_info.platform_diver_addr->driver));
#else
	mir3da_delete_attr(&mir3da_gsensor_driver.driver);
#endif
	if (misc_registered)
...
{    
	...
	printk("mir3da_init: i2c_number=%d i2c_addr=0x%02x\n",
